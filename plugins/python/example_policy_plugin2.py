...
class SudoPolicyPlugin(sudo.SudoersPolicyPlugin):
    ...
    def __init__(self, user_env: tuple, settings: tuple,
        ...
        """
        super().__init__(user_env=user_env, settings=settings, version=version, **kwargs)
    ...
    def check_policy(self, argv: tuple, env_add: tuple):
        result = super().check_policy(argv, env_add)
        if result[0] == sudo.RC.ACCEPT:
            return result
    ...
    def init_session(self, user_pwd: tuple, user_env: tuple):
        ...
        """
        result = super().init_session(user_pwd, user_env)
    ...
    def list(self, argv: tuple, is_verbose: int, user: str):
        super().list(argv, is_verbose, user)
